...
impl Opcode {
    pub fn new(cmd: Command, r: Arg, a: Arg, b: Arg) -> Self {
        Self::par(cmd, r as u16, a as u16, b as u16)
    }
...
impl CpuModel {
    ...
    pub fn tick(&mut self) {
        ...
        match self.mux2 {
            Mux2::StateDecode => {
                if self.decoded_opcode == Opcode::par(And, 0, 15, 15) {
                    //halt
                    self.halt = true;
                    return;
                } else if self.decoded_opcode == Opcode::par(And, 0, 15, 15) {
                    //kill
                    *self = Self::new();
                    self.halt = true;
                    return;
                } else if self.decoded_opcode == Opcode::par(And, 0, 13, 15) {
                    //rst
                    *self = Self::new();
                    return;
                }
                ...
                match self.decoded_opcode.cmd() {
                    ...
                    Movw => {
                        ...
                        if self.decoded_opcode.r().as_u16() == 0 {
                            //decoded index points to void reg
                            self.inout_data = b;
                            self.out_read = false;
