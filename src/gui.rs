use crate::emulator::{Command, Mux2, Opcode, SubCommand};
...
impl RamView {
    pub fn new() -> Self {
        ...
        //ViewSwitcher::new()
        Self { prev_size: (0, 0), prev_scroll: 0, view: WidgetPod::new(wg.expand().background(Color::LIME)).boxed() }
    }
    ...
    fn label_for(index: usize, color: Color) -> impl Widget<GuiData> {
        ...
        Container::new(Align::centered(label).fix_size(W_CELL, H_CELL).background(Painter::new(
            move |ctx, data: &GuiData, _env| {
                let cpu = data.vm.cpu();
                ...
                match cpu.decoded_opcode.cmd() {
                    ...
                    Command::Ex
                        if cpu.decoded_opcode.b_sub() == SubCommand::Cldi
                            && (cpu.mux2 == Mux2::StateSaveToReg(false) || cpu.mux2 == Mux2::StateSkip) =>
                    {
                        if index == pc {
                    ...
                    Command::Ex
                        if cpu.decoded_opcode.b_sub() == SubCommand::Call
                            && (cpu.mux2 == Mux2::StateSaveToReg(true) || cpu.mux2 == Mux2::StateSkip) =>
                    {
                        if index == pc {
                            if cpu.mux2 == Mux2::StateSkip {
                                ctx.fill(rect, &COL_EX_INSTR);
    ...
    fn create_layout(width: usize, height: usize, pos: usize) -> impl Widget<GuiData> {
        ...
        for i in 0..width {
            top_row.add_child(
                Align::centered(Label::new(format!("{:0d$X}", i, d = digits)))
                    .fix_size(W_CELL, H_CELL)
                    .background(if i % 2 == 0 { COL_ADR_EVEN } else { COL_ADR_ODD }),
            );
        }
        ...
        for y in 0..height {
            ...
            row.add_child(
                Align::centered(Label::new(format!("{:04X}", ram_index)))
                    .fix_size(W_CELL, H_CELL)
                    .background(if y % 2 == 0 { COL_ADR_EVEN } else { COL_ADR_ODD }),
            );
            for _ in 0..width {
...
fn bits_view() -> impl Widget<u16> {
    ...
    .controller(BitsControl);

    Flex::row().with_child(lab.padding(Insets { x0: 10.0, x1: 5.0, ..Insets::default() })).with_child(bits)
}
fn build_cpu_state_view() -> impl Widget<GuiData> {
    ...
    let mut regs = Flex::column().with_child(bits_view().lens(Map::new(|_: &GuiData| 0, |_: &mut GuiData, _| {})));
    for i in 1..=15 {
        let lens = Map::new(
            move |g: &GuiData| g.vm.cpu().reg[i],
            move |g: &mut GuiData, v| {
                g.vm.cpu_mut().reg[i] = v;
            },
        );
